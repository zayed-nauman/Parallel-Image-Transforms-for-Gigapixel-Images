...
//
//  Extends Milestone 1 CLI with GPU/heterogeneous modes.
//  All M1 modes (sequential, parallel, both) still work unchanged.
//  New modes: --mode hetero | hetero-bench
// ─────────────────────────────────────────────────────────────────────────────
...
static void print_usage(const char* prog) {
    std::cout <<
        ...
        "                         Without this flag all tiles go to CPU.\n"
        "  --gpu-tile-size <N>    GPU tile size (default 1024)\n"
        "  --cpu-tile-size <N>    CPU tile size (default 512)\n"
        "  --streams <N>          Number of CUDA streams (default 3)\n"
        "\n"
        "Options:\n"
        "  --transform <name>     Add a transform (repeatable)\n"
...
int main(int argc, char* argv[]) {
    ...
    while (i < argc) {
        ...
        if (arg == "--mode" && i + 1 < argc) {
            ...
            if (tname == "identity") {
                ...
                chain.add(std::make_unique<BoxBlurTransform>(parse_int(argv[++i])));
                ...
                chain.add(std::make_unique<RotateTransform>(parse_float(argv[++i])));
            } else {
    ...
    try {
        ...
        // Compute correct output dimensions after the full transform chain.
        // BUG FIX: previously TileWriter was given info.width/height (input size)
        // so resize/crop wrote output files of the wrong size.
        uint32_t out_w, out_h;
        chain.compute_output_size(info.width, info.height, out_w, out_h);
        ...
        if (mode == "sequential" || mode == "both") {
            ...
            std::cout << "\n── Running SEQUENTIAL ────────────────────────────\n";
            TileWriter seq_writer(out, out_w, out_h, info.fmt, 256);
            ...
            // Note: [SequentialProcessor] header is printed inside seq.run()
        ...
        if (mode == "parallel" || mode == "both") {
            ...
            TileWriter par_writer(output_path, out_w, out_h, info.fmt, 256);
            ...
            std::cout << "[TileProcessor] " << actual_threads << " worker threads, "
                      << reader.num_tile_cols(cfg.tile_size) << "x"
                      << reader.num_tile_rows(cfg.tile_size) << " tile grid ("
                      << s.tiles_read << " tiles), halo="
                      << std::max(cfg.halo_size, chain.max_halo()) << "\n\n"
        ...
        if (mode == "hetero" || mode == "hetero-bench") {
            if (!hcfg.gpu_available)
                std::cout << "  NOTE: pass --gpu to enable GPU routing. "
                             "All tiles will go to CPU.\n";
#ifndef HAVE_CUDA
            if (hcfg.gpu_available)
                std::cout << "  NOTE: CUDA not compiled in. "
                             "GPU queue uses CPU-stub kernels.\n";
#endif
            std::cout << "\n── Running HETEROGENEOUS (CPU+GPU) ───────────────\n";
            TileWriter het_writer(output_path, out_w, out_h, info.fmt, 256);
        ...
        if (mode == "hetero-bench") {
            ...
            TileWriter par2_writer(out2, out_w, out_h, info.fmt, 256);
